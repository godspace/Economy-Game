...
export async function showDealModal(userId) {
    try {
        ...
        // ИСПРАВЛЕНИЕ: Проверяем, что у пользователя есть хотя бы 1 монета для резервирования
...
export async function proposeDeal(choice) {
    try {
        ...
        // ИСПРАВЛЕНИЕ: Проверяем, что у пользователя есть минимум 1 монета для резервирования
        if (state.currentUserProfile.coins < 1) {
            alert('У вас недостаточно монет для совершения сделки. Требуется минимум 1 монета для резервирования.');
            if (dom.dealModal) {
        ...
        // ИСПРАВЛЕНИЕ: Используем RPC функцию для атомарного создания сделки с резервированием 1 монеты
...
export async function respondToDeal(choice) {
    try {
        ...
        // ИСПРАВЛЕНИЕ: Используем RPC функцию для атомарной обработки сделки с возвратом резервной монеты
...
async function showDealResult(deal, userChoice, result) {
    try {
        ...
        // ИСПРАВЛЕНИЕ: Добавляем информацию о возврате резервной монеты
...
// [Здесь должен быть остальной код из предыдущей версии deals.js: loadDeals, renderDeals, renderDealsList, renderCompletedDeals, loadRanking, renderRanking]

// Добавляем функцию для обновления баланса пользователя
...
// [ВСТАВИТЬ ЗДЕСЬ ВЕСЬ ОСТАВШИЙСЯ КОД ИЗ ПРЕДЫДУЩЕЙ ВЕРСИИ deals.js]
// Функции loadDeals, renderDeals, renderDealsList, renderCompletedDeals, loadRanking, renderRanking
// должны быть скопированы из предыдущей версии файла без изменений